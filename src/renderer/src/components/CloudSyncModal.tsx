...
export function CloudSyncModal(): React.JSX.Element {
  ...
  async function applyEncryption() {
    ...
    setEncOpen(false)
    setPassphrase('')
    toast.success('已开启端到端加密，请在其它设备设置相同口令')
  }
  ...
  function afterConnect(ok: boolean, name: string) {
    if (ok) {
      ...
      toast.error('连接失败，请检查配置与凭证')
    }
  }

  async function handleSync() {
    const r = await runSync()
    if (r.status === 'error') toast.error(r.message || '同步失败')
    else toast.success(r.message || '同步完成')
  }
  ...
  async function handleRestore(v: SyncVersion) {
    if (!confirm(`恢复到 ${formatDate(v.createdAt)} 的版本？当前数据会被替换。`)) return
    const r = await restoreSyncVersion(v.id)
    if (r.status === 'error') toast.error(r.message || '恢复失败')
    else {
                ...
                return (
                  <div key={prov.id} className="rounded-2xl border border-line-strong bg-surface p-4">
                      ...
                      {connected ? (
                        <div className="flex items-center gap-1 text-sm">
                          <CardAction
                            ...
                            disabled={syncBusy}
                          />
                          <CardAction icon={<History size={15} />} label="历史版本" onClick={openHistory} />
                          <button
                            onClick={async () => {
                    ...
                    {connected && (
                      <div className="mt-3 flex items-center justify-between border-t border-line pt-3">
                        ...
                        <Switch
                          ...
                          onChange={async (v) => {
                            await setAutoSync(v)
                            toast.success(v ? '已开启自动同步' : '已关闭自动同步')
                          }}
                        />
                      </div>
                    )}
                    ...
                    {connected && (
                      <div className="mt-3 border-t border-line pt-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-sm text-ink">端到端加密</div>
                            <div className="text-xs text-faint">
                    ...
                    {!connected && isConnecting && prov.id === 'gist' && (
                      <ConnectForm>
                        <Label>
                          GitHub Personal Access Token（需勾选 <code className="var-chip">gist</code> 权限）
                        </Label>
                    ...
                    {!connected && isConnecting && prov.id === 'webdav' && (
                      <ConnectForm>
                        <Label>WebDAV 地址（指向一个目录，如坚果云 https://dav.jianguoyun.com/dav/PromptBox/）</Label>
                        <input
                          value={dav.url}
                    ...
                    {!connected && isConnecting && prov.id === 's3' && (
                      <ConnectForm>
                        <Label>S3 兼容存储（AWS S3 / MinIO / Cloudflare R2 / Backblaze B2 等）</Label>
                        <input
                          value={s3.endpoint}
                          onChange={(e) => setS3({ ...s3, endpoint: e.target.value })}
                          placeholder="Endpoint，如 https://s3.amazonaws.com 或 https://xxx.r2.cloudflarestorage.com"
                          className={inputCls + ' mb-2'}
                        />
                        <div className="mb-2 flex gap-2">
                          <input
                        ...
                        <input
                          ...
                          placeholder="路径前缀（可选，如 promptbox/）"
                          className={inputCls + ' mb-2'}
                        />
                        <ConnectBtn
                          full
...
function StatusPanel(): React.JSX.Element {
  ...
  if (!syncState?.connected) {
    return <div className="px-2 py-10 text-center text-sm text-faint">尚未连接任何云服务。</div>
  }
  const status = syncState.lastStatus ?? 'idle'
  const providerName = SYNC_PROVIDERS.find((p) => p.id === syncState.provider)?.name ?? '—'
  return (
    <div className="space-y-3 text-sm">
      <Field label="云服务">{providerName}</Field>
      <Field label="账号">{syncState.account ?? '—'}</Field>
      <Field label="状态">
        <span
      ...
      {syncState.lastMessage && <Field label="详情">{syncState.lastMessage}</Field>}
      <Field label="上次同步">
        {syncState.lastSyncedAt ? formatDate(syncState.lastSyncedAt) : '尚未同步'}
      </Field>
      <Field label="本地条目">{prompts.length} 个 Prompt</Field>
      <Field label="设备 ID">
...
function HistoryPanel({
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="py-8 text-center text-sm text-faint">加载中…</div>
      ) : versions.length === 0 ? (
        <div className="py-8 text-center text-sm text-faint">暂无历史版本。</div>
      ) : (
