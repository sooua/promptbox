...
import { Check, Compass, Download, Loader2, Plug, Search, WifiOff } from 'lucide-react'
import type { McpDiscoverItem } from '@shared/types'
import { useStore } from '../store'
import { useT } from '../i18n'
import { toast } from './Toast'

type Tab = 'mcp' | 'skill' | 'agent'
    ...
    <div className="flex flex-1 flex-col bg-canvas">
      <div className="border-b border-line px-8 pt-7">
        ...
        <div className="flex items-center gap-1">
          ...
          <TabBtn active={tab === 'skill'} onClick={() => setTab('skill')} soon>
            Skill
          </TabBtn>
          <TabBtn active={tab === 'agent'} onClick={() => setTab('agent')} soon>
            Agent
          </TabBtn>
        </div>
      </div>
...
function McpDiscover(): React.JSX.Element {
  ...
  async function run(q: string, cur?: string) {
    const id = ++reqId.current
    setLoading(true)
    setError(null)
    const r = await searchMcp(q, cur)
  ...
  async function doImport(item: McpDiscoverItem) {
    ...
    const r = await importMcp(item.server)
    setImporting(null)
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="flex items-center gap-2 px-8 py-4">
        ...
        <span className="ml-auto text-[11px] text-faint">{t('来自官方 MCP 注册表')}</span>
      </div>
...
function ComingSoon(): React.JSX.Element {
  const t = useT()
  return (
    <div className="flex flex-1 items-center justify-center text-sm text-faint">{t('即将支持')}</div>
  )
}
