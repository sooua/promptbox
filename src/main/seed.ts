...
写到 docs/tasks/<功能名>.md。先不要写代码，等我确认任务列表。`
  },
  {
    stage: 'build',
    name: '实现',
    hint: '按任务清单一个一个做，做完一个打一个勾。在 Claude Code 里说「继续」做下一个，全部勾完再点下一步。',
    output: '代码 + 勾选的任务',
    prompt: `读 docs/tasks/ 里当前功能的任务文件。

按顺序做第一个未勾选的任务：先读相关文件，改动最小，遵循现有风格。做完把它勾上、git commit 一次，告诉我改了哪些文件、怎么验证。然后停下等我说「继续」。`
  },
  {
    stage: 'build',
    name: '测试',
...
为这个功能写测试：正常路径、边界、出错情况。测试名要能读出「什么情况下期望什么」。跑一遍，把结果贴给我。哪些行为不值得测，说明理由。`
  },
  {
    ...
    output: '问题清单 + 修复',
    prompt: `审查这个功能开始以来的所有改动（用 git log 找到对应的提交，git diff 看差异）。
